...
// === ЧАТ-БОТ ===
// =====================================================================
// === ТВОЄ: базовий примітивний чат-бот зі словником точних відповідей
// =====================================================================
const botResponses = {
...
// =====================================================================
// === МОЄ (НОВЕ): розширена база знань з ключовими словами
// =====================================================================
const baseKnowledge = [
  { keys: ["скільки", "вершин"], answer: "24 — тому що 4! = 24 перестановки." },
  { keys: ["що", "таке", "ребро"], answer: "Ребро з'єднує дві перестановки, які відрізняються транспозицією сусідніх елементів." },
  { keys: ["скільки", "граней"], answer: "14: 8 трикутних і 6 чотирикутних." },
  { keys: ["центр"], answer: "Центр — точка (2.5, 2.5, 2.5)." },
];

// =====================================================================
// === МОЄ (НОВЕ): можливість навчати бота через команду /teach
// =====================================================================
...
// =====================================================================
// === МОЄ (НОВЕ): "розумний" пошук відповіді (ключові слова + рейтинг)
// =====================================================================
function findSmartAnswer(text) {
  text = text.toLowerCase();

  // 1) Перевірка, чи бот раніше навчений
  ...
  for (const item of baseKnowledge) {
    ...
    for (const key of item.keys) {
      if (text.includes(key)) score += 2;              // точне попадання
      for (const w of words) {
        if (w.startsWith(key)) score += 1;             // часткове
      }
    }
...
// =====================================================================
// === МОЄ (НОВЕ): команда навчання бота /teach питання = відповідь
// =====================================================================
...
// =====================================================================
// === ТВОЄ + МОЄ: я залишив твою структуру initChat, але вміст покращив
// =====================================================================
function initChat() {
  // === ТВОЄ: генерування базової верстки чату ===
  const content = $('#content');
  content.innerHTML = `
  ...
  // === ТВОЄ + МОЄ: твоя логіка відправки, але з розумною відповіддю ===
  $('#send').onclick = () => {
    ...
    if (teach) {
      setTimeout(() => addMsg(teach, 'bot'), 300);
      input.value = '';
      return;
    }

    // === МОЄ: розумний пошук відповіді ===
    const answer = findSmartAnswer(txt);

    if (answer) {
      setTimeout(() => addMsg(answer, 'bot'), 300);
    } else {
      // === МОЄ: пропозиція навчити бота ===
      setTimeout(() => addMsg(
        "Я ще не знаю цієї відповіді 💭. Навчи мене:<br><b>/teach питання = відповідь</b>",
        'bot'
      ), 300);
    }
